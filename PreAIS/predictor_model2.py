...
import torch
from model.model2 import model_DNN_CNN
from feature_extract.BPB import bpb_extract_features
...
def accept(file_path):
    ...
    data = bpb_extract_features(seq, None)
    model = model_DNN_CNN(data.shape[1], 1)
    model.load_state_dict(torch.load('./ckpt_save/best2.pth'))
    model.eval()
